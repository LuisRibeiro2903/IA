...
leitoresF(L) :- findall(_, leitores(_, _, f), T), tamanho(T, L).

%--------------------------------- - - - - - - - - - -  -  -  -  -   -
% II

livrosReq(L) :- findall(Nomes, (requisicoes(_, _, Num, _), livros(Num, Nomes, Bib), not(biblioteca(Bib, _, _))), L).

%--------------------------------- - - - - - - - - - -  -  -  -  -   -
% III

livros_leitores(L) :- findall((Leitor, Livro), (requisicoes(_, IdLeitor, IdLivro, _), leitores(IdLeitor, Leitor, Bib), livros(IdLivro, Livro, Bib), biblioteca(Bib, _, braga)), L).